...
public class ViewClock extends View {
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        canvas.drawCircle(x, y, r, paint);        //绘制外圆
        canvas.drawCircle(x, y, 15, paintMinute);        //绘制圆心
        drawLines(canvas);        //绘制刻度
        drawText(canvas);        //绘制整点
        try {
            initCurrentTime(canvas);
        } catch (Exception e) {
            e.printStackTrace();
        }
        postInvalidateDelayed(1000);        //每隔1s刷新界面
    }
